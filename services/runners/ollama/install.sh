...
if [ -x "ollama" ]; then
    ...
    if [ "$CURRENT_VERSION" = "$OLLAMA_VERSION" ]; then
        ...
        # ==============================================================================
        # We strip literal quotes generated by orchestrate.py and parse the array using IFS
        # to safely iterate the targets.
        ...
        for model in "${models[@]}"; do
            if [[ "$model" == *"ingu627/llama4-scout-q4:109b"* ]]; then
                ...
                # HISTORICAL CONTEXT: We use a custom Modelfile to explicitly enforce the Llama 4
                # boundary tags. Without these, Ollama gets confused during structured JSON sampling
                # and hallucinates "assistant\n\n" directly into the tool call payload.
                # Note: Overriding the TEMPLATE breaks Ollama's internal regex tool parser, which
                # is why the model outputs raw JSON as a `content` string instead of a `tool_calls` array.
