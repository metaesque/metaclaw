...
if [ -d "/usr/local/lib/ollama" ]; then
    echo "WARNING: Global Ollama library directory detected at /usr/local/lib/ollama. Destroying..."
...
# HISTORICAL CONTEXT: We previously used `rm -rf ../lib && mv lib ../` which catastrophically
# deleted the framework's core `metaclaw.py` library. This explicit targeted extraction is required.
mkdir -p ../lib
