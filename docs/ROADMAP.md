...
5. **Refactor `patch_routing.py` (The CLI Refactor):** Rewrite the script so that it configures OpenClaw using official CLI commands (`openclaw config set`, `openclaw agents add`) rather than brittle, direct manipulation of `openclaw.json`. *CRITICAL REQUIREMENT: The new Python script must inject detailed comments/documentation into the generated OpenClaw Bash execution scripts, explaining the security ramifications and context of each CLI command.*
...
12. **Generalize Agent Domains:** Review and rewrite the domains and mandates of all teams and agents (e.g., `sre_lead`) so they are less MetaClaw-specific and broad enough to capture generic sysadmin, coding, and networking tasks.
13. **Implement Health Assessment / Alerting Engine:** Add a dedicated health-monitoring pipeline (or agent skill) capable of verifying container states on headless Tier 1/Tier 2 nodes, aggregating logs accurately per-service, and alerting the user of crash loops without requiring manual `docker ps` execution.
