...
* **Path**: `services/collectors/`
* **Purpose**: A unified agent running on every node that executes scripts and
  scrapes local hardware metrics, securely transmitting them to the central
  TSDB.
* **Options**: OpenTelemetry Collector, Telegraf, Datadog Agent, Grafana Alloy,
  Prometheus Agent, Vector.

### Log Aggregator {#loggers}

* **Path**: `services/loggers/`
* **Purpose**: The central database storage engine managing structured records,
  analytical data pools, and log metrics for retrospective full-text query
  pipelines.
* **Options**: Grafana Loki, Graylog, OpenSearch, Quickwit, VictoriaLogs.

### Data Visualizer {#visualizers}

* **Path**: `services/visualizers/`
* **Purpose**: The UI layer for querying and displaying metric and log data.
  Connects to TSDBs and Log Aggregators to provide real-time analytical
  dashboards.
* **Options**: SigNoz, Kibana, Perses, Chronograf, Grafana, OpenObserve.

### Telemetry Forwarder {#forwarders}

* **Path**: `services/forwarders/`
* **Purpose**: Lightweight edge daemons that process local container logs and
  bare-metal files on the fly, streaming parsed metrics and JSON buffers
  directly back to the active log aggregator backend.
* **Options**: Logstash Collector, Fluent Bit Mesh Agent, Vector Pipeline.

### Time-Series Database {#tsdbs}

* **Path**: `services/tsdbs/`
* **Purpose**: The central storage and query engine optimized specifically for
  high-frequency, structured numerical metrics (e.g., CPU temperature, Power
  wattage). It periodically scrapes localized edge exporters.
* **Options**: VictoriaMetrics, InfluxDB, M3DB, Prometheus, QuestDB, GreptimeDB,
  TimescaleDB.
